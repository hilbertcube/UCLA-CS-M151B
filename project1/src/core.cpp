...
Core::Core(const SimContext& ctx, uint32_t core_id, ProcessorImpl* processor)
    : SimObject(ctx, "core"), core_id_(core_id), processor_(processor), reg_file_(NUM_REGS)
{
    this->reset();
}
...
void Core::ex_stage()
{
    ...
    // ALU operations
    auto result =
        this->alu_unit(*stage_data.instr, stage_data.rs1_data, stage_data.rs2_data, stage_data.PC);

    // Branch operations
    result = this->branch_unit(*stage_data.instr, stage_data.rs1_data, stage_data.rs2_data, result,
                               stage_data.PC);

    DT(2, "EX: result=0x" << std::hex << result << std::dec << " (#" << stage_data.uuid << ")");

    // move instruction data to next stage
    ex_mem_.push({stage_data.instr, stage_data.rs1_data, stage_data.rs2_data, result, stage_data.PC,
                  stage_data.uuid});
    id_ex_.pop();
}
...
bool Core::check_data_hazards(const Instr& instr)
{
    ...
    if (!ex_mem_.empty())
    {
        ...
        // Check for load-use hazard: if EX/MEM stage has a LOAD instruction
        // and current instruction uses the load destination as source
        if (ex_exe_flags.is_load && ex_exe_flags.use_rd && ex_instr.getRd() != 0)
        {
            uint32_t rd = ex_instr.getRd();
            // Check if current instruction uses this register as source
            if ((exe_flags.use_rs1 && instr.getRs1() == rd) ||
                (exe_flags.use_rs2 && instr.getRs2() == rd))
            {
                DT(2, "*** Load-use hazard detected: rd=" << rd << " (#" << if_id_.data().uuid << ")");
...
bool Core::data_forwarding(uint32_t reg, uint32_t* data)
{
    ...
    if (!ex_mem_.empty())
    {
        auto& ex_data = ex_mem_.data();
        auto& ex_instr = *ex_data.instr;
        // TODO: check data forwarding from EX/EM
        auto ex_exe_flags = ex_instr.getExeFlags();

        // Forward from EX/MEM stage if instruction writes to rd and rd matches reg
        if (ex_exe_flags.use_rd && ex_instr.getRd() != 0 && ex_instr.getRd() == reg)
        {
            *data = ex_data.result;
            forwarded = true;
            DT(2, "Forwarding from EX/MEM: reg=" << reg << ", data=0x" << std::hex << *data
            << std::dec << " (#" << if_id_.data().uuid << ")");
        }
    }
    ...
    if (!forwarded && !mem_wb_.empty())
    {
        ...
        // Forward from MEM/WB stage if instruction writes to rd and rd matches reg
        if (mem_exe_flags.use_rd && mem_instr.getRd() != 0 && mem_instr.getRd() == reg)
        {
            *data = mem_data.result;
            forwarded = true;
            DT(2, "Forwarding from MEM/WB: reg=" << reg << ", data=0x" << std::hex << *data
            << std::dec << " (#" << if_id_.data().uuid << ")");
        }
    }
...
void Core::regfile_read(const Instr& instr, uint32_t* rs1_data, uint32_t* rs2_data)
{
    ...
    if (exe_flags.use_rs1 && instr.getRs1() != 0)
    {
        if (!this->data_forwarding(instr.getRs1(), &_rs1_data))
        {
            ...
            DT(2, "Regfile: addr=" << instr.getRs1() << ", data=0x" << std::hex << _rs1_data
                                   << std::dec << " (#" << if_id_.data().uuid << ")");
        }
    }
    ...
    if (exe_flags.use_rs2 && instr.getRs2() != 0)
    {
        if (!this->data_forwarding(instr.getRs2(), &_rs2_data))
        {
            ...
            DT(2, "Regfile: addr=" << instr.getRs2() << ", data=0x" << std::hex << _rs2_data
            << std::dec << " (#" << if_id_.data().uuid << ")");
        }
    }
    ...
    if (exe_flags.is_csr)
    {
        ...
        DT(2, "CSR: addr=0x" << std::hex << instr.getImm() << ", data=0x" << _rs2_data << std::dec
        << " (#" << if_id_.data().uuid << ")");
    }
