...
static const std::unordered_map<Opcode, InstType> sc_instTable = {
    {Opcode::R, InstType::R},     {Opcode::L, InstType::I},     {Opcode::I, InstType::I},
    {Opcode::S, InstType::S},     {Opcode::B, InstType::B},     {Opcode::LUI, InstType::U},
    {Opcode::AUIPC, InstType::U}, {Opcode::JAL, InstType::J},   {Opcode::JALR, InstType::I},
    {Opcode::SYS, InstType::I},   {Opcode::FENCE, InstType::I},
};
...
std::ostream& operator<<(std::ostream& os, const Instr& instr)
{
    ...
    os << std::dec << ", alu_op=" << instr.getAluOp() << ", br_op=" << instr.getBrOp()
...
std::shared_ptr<Instr> Core::decode(uint32_t instr_code) const
{
    ...
    {
        std::cout << std::hex << "Error: invalid opcode: 0x" << static_cast<int>(opcode)
                  << std::endl;
        return nullptr;
    }
    ...
    case InstType::I:
    {
        switch (opcode)
        {
        case Opcode::I: // I-type instructions
            exe_flags.use_rd = 1;
            exe_flags.use_rs1 = 1;
            exe_flags.use_imm = 1;
        ...
        case Opcode::SYS:
        {
            ...
            // bug here, imm = 0
            // I-type immediate: bits [31:20] encode the CSR address (or system function)
    ...
    case InstType::B:
    {
        ...
        uint32_t imm_12 = (instr_code >> 31) & 0x1;    // bit 31 -> im[12]
        uint32_t imm_10_5 = (instr_code >> 25) & 0x3F; // bits 30-25 (6 bits) -> imm[10:5]
        uint32_t imm_4_1 = (instr_code >> 8) & 0xF;    // bits 11-8 (4 bits) -> imm[4:1]
        uint32_t imm_11 = (instr_code >> 7) & 0x1;     // bit 7 -> imm[11]

        uint32_t imm_raw = (imm_12 << 12) | (imm_11 << 11) | (imm_10_5 << 5) | (imm_4_1 << 1);
    ...
    case InstType::U:
    {
        ...
        uint32_t imm_31_12 = (instr_code >> 12) & 0xFFFFF;
        imm = imm_31_12 << 12;
    }
    break;
    ...
    case InstType::J:
    {
        ...
        //   imm[0] is implicitly 0 (word aligned -- technically 2 byte alligned actually), so we
        //   shift left by 1.
