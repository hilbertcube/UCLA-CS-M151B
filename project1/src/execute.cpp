...
uint32_t Core::alu_unit(const Instr &instr, uint32_t rs1_data, uint32_t rs2_data, uint32_t PC) {
  ...
  case AluOp::MUL: {
    rd_data = static_cast<int32_t>(alu_s1) * static_cast<int32_t>(alu_s2);
    break;
  }
  case AluOp::MULH: {
  ...
  case AluOp::SLL: {
    // Shift Left Logical) instruction shifts the bits in a register (rs1) to the left by a value specified in another register (rs2)
    rd_data = // TODO:
    break;
  }
  case AluOp::SRL: {
