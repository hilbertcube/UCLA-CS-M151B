...
uint32_t Core::alu_unit(const Instr& instr, uint32_t rs1_data, uint32_t rs2_data, uint32_t PC)
{
    ...
    case AluOp::MULH:
    {
        // upper 32 bits, rs1 is signed, rs2 is signed
        rd_data = (static_cast<int64_t>(alu_s1) * static_cast<int64_t>(alu_s2)) >> 32;
        break;
    }
    case AluOp::MULHSU:
    {
        // Upper 32 bits, rs1 is signed, rs2 is unsigned
        rd_data = ((int64_t) (int32_t) alu_s1 * (uint64_t) alu_s2) >> 32;
        break;
    }
    case AluOp::MULHU:
    {
        // MULHU = unsigned * unsigned
        rd_data = ((uint64_t) alu_s1 * (uint64_t) alu_s2) >> 32;
        break;
    }
    case AluOp::AND:
    {
    ...
    case AluOp::SLL:
    {
        // Shift Left Logical instruction shifts the bits in a register (rs1) to the left by a value
        // specified in another register (rs2)
        rd_data = alu_s1 << (alu_s2 & 0x1f); // 0001 1111, same as alu_s2 % 32
        break;
    }
    case AluOp::SRL:
    {
...
uint32_t Core::branch_unit(const Instr& instr, uint32_t rs1_data, uint32_t rs2_data,
                           uint32_t rd_data, uint32_t PC)
{
    ...
    if (br_op != BrOp::NONE)
    {
        ...
        DT(2, "Branch: " << (br_taken ? "taken" : "not-taken") << ", target=0x" << std::hex
        << br_target << std::dec << " (#" << id_ex_.data().uuid << ")");
    }
...
void Core::dmem_read(void* data, uint64_t addr, uint32_t size)
{
    ...
    DTH(2, "Mem Read: addr=0x" << std::hex << addr << ", data=0x" << ByteStream(data, size)
    << " (size=" << size << ", type=" << type << ")");
}
...
void Core::dmem_write(const void* data, uint64_t addr, uint32_t size)
{
    auto type = get_addr_type(addr);
    __unused(type);
    if (addr >= uint64_t(IO_COUT_ADDR) && addr < (uint64_t(IO_COUT_ADDR) + IO_COUT_SIZE))
    {
    ...
    DTH(2, "Mem Write: addr=0x" << std::hex << addr << ", data=0x" << ByteStream(data, size)
     << " (size=" << size << ", type=" << type << ")");
}
...
void Core::set_csr(uint32_t addr, uint32_t value)
{
    ...
    default:
    {
        std::cout << std::hex << "Error: invalid CSR write addr=0x" << addr << ", value=0x" << value
                  << std::endl;
        std::abort();
    }
    }
