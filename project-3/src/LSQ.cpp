...
LoadStoreQueue::LoadStoreQueue(const SimContext &ctx, const char *name, uint32_t size, Core *core)
    : SimObject<LoadStoreQueue>(ctx, name), store_(size), head_index_(0), tail_index_(0), count_(0), core_(core), perf_stats_()
{
  this->reset();
}
...
bool LoadStoreQueue::analyze_load_dependencies(uint32_t load_idx, bool *can_forward, uint32_t *fwd_data)
{
  ...
  *fwd_data = 0;
  ...
  for (uint32_t i = 0; i < store_.size(); ++i)
  {
    ...
    {
      bool is_candidate = older.data_valid && older.addr_valid && older.addr == load.addr && older.instr->getFunc3() == load.instr->getFunc3() && (older.state == LoadStoreQueue::lsq_state_t::Writeback || older.state == LoadStoreQueue::lsq_state_t::Committed);
      if (is_candidate)
      {
